...
function initEnv() {
  ...
  assert(
    path.isAbsolute(config.releaseDestDir) &&
...
async function md2jsonAsync(opt) {
  ...
  return await new Promise((resolve, reject) => {
    ...
    if (argv.watch) {
      chokidar
        .watch(path.resolve(__dirname, opt.language, opt.entry), {
          ignoreInitial: true,
        })
        .on("all", (event, path) => {
...
function copyAsset() {
  ...
  function doCopy() {
    for (let lang of languages) {
      const assetDestDir = path.resolve(
...
function copyWebsiteStatic() {
  const staticDires = [
    "asset",
    "fastly.jsdelivr.net",
    "registry.npmmirror.com",
  ];
  const staticSrcDir = path.resolve(projectDir, "../echarts-website-static");
  const staticDestDir = path.resolve(config.releaseDestDir);

  function doCopy() {
    // for (let static of staticDires) {
    //   const staticSrcDirIn = path.resolve(staticSrcDir, `${static}`);
    //   copydir.sync(staticSrcDirIn, staticDestDir);
    // }
    copydir.sync(staticSrcDir, staticDestDir);
    console.log(staticSrcDir, staticDestDir);
  }
  var doCopyDebounced = debounce(doCopy, 500, {
    leading: false,
    trailing: true,
  ...
  console.log("Copy echarts-website-static done.");
}
...
function buildCodeStandard() {
  ...
  for (let lang of languages) {
    const codeStandardDestPath = path.resolve(
      config.ecWWWGeneratedDir,
