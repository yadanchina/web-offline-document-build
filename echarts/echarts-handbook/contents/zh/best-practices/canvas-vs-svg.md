...
ECharts 从初始一直使用 Canvas 绘制图表。而 [ECharts v4.0](/zh/changelog.html#v4-0-0) 发布了 SVG 渲染器，从而提供了一种新的选择。在初始化图表实例时，只需设置 [renderer 参数](${mainSitePath}api.html#echarts.init) 为 `'canvas'` 或 `'svg'` 即可指定渲染器，比较方便。
